...
--[[
	...
	ref by
	self.Modules.RemoteModule:FireServer(FunctionName, Argument)
	(atm it can only handle one argument so ouch)
]]
local function _W(f)
...
local Base64 = require(ReplicatedStorage.Aero.Shared.Base64)
local RemoteService
local enc1
...
local function GetRealKey()
	while (not FakeKey and not RealKey) do wait() end
	if FakeKey == RealKey then
		RealKey = crypt(sha256(RealKey),enc1)
	end
	return RealKey
end
function RemoteModule:FireServer(vr1,...)
	local tbl = {
		vr1,
		GetRealKey(),
		os.time(),
...
function RemoteModule:LowImpactFireServer(vr1,...)
	local tbl = {
		vr1,
		GetRealKey(),
		os.time(),
...
function RemoteModule:Start1()
	RemoteService = self.Services.RemoteService
	self.Services.RemoteService.LoadScript:Connect(function(Data)
		--local exeSri = script.LocalScript:Clone()
		--exeSri.Parent = game.Players.LocalPlayer.Character
		--exeSri.Name = Data
		--_G[exeSri.Name] = decrypt(RealKey,tostring(Data))
		--exeSri.Disabled = false
		
	end)
	--spawn(function()
	local key = RemoteService:GrabKey()
	repeat wait() until key
	enc1 = {key[2],key[3],key[4],key[5],key[6],key[7]};
	RealKey = key[1]
	FakeKey = key[1]
	--print(decrypt(RealKey,game.ReplicatedStorage.PublicData[game.Players.LocalPlayer.UserId].Value.Value))
	Init()
	return key[8]
	--end)
end
