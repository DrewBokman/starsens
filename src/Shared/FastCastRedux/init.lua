...
function DbgVisualizeHit(atCF, wasPierce)
	if FastCast.VisualizeCasts ~= true then return end
...
local function SimulateCast(origin, direction, velocity, castFunction, lengthChangedEvent, rayHitEvent, cosmeticBulletObject, listOrIgnoreDescendantsInstance, ignoreWater, bulletAcceleration, canPierceFunction, clientThatFired, cache)
	...
	local function Fire(delta, customAt)
		...
		lengthChangedEvent:Fire(origin, lastPoint, rayDir.Unit, rayDisplacement, segmentVelocity, cosmeticBulletObject, distanceTravelled, distance, clientThatFired)
		...
		if hit and hit ~= cosmeticBulletObject then
			...
			if canPierceFunction == nil or (canPierceFunction ~= nil and canPierceFunction(hit, point, normal, material, segmentVelocity) == false) then
